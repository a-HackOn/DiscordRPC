...
try:
    ...
    discord_rpc.initialize("926512280743579668", callbacks=callbacks, log=True)     # Discord Developer Portal -> RPC if you cannot understand this (obv if you want)
    discord_rpc.update_presence(**{
        'details': f"Logged in as ["+name+"] | "+str(pid)+" ",
        'state': "ONE STEP AHEAD OF THE GAME",
        'start_timestamp': time.time(),
        'large_image_key': "aimware",
        'large_image_text': 'Pain'
    })
    for i in range(3):      
