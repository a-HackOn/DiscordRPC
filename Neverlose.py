...
try:
    ...
    pid = pid * 17.586
    pid = round(pid)
    discord_rpc.initialize("926582429639147600", callbacks=callbacks, log=True)     # Discord Developer Portal -> RPC if you cannot understand this (obv if you want)
    discord_rpc.update_presence(**{
        'details': f"Logged in as ["+name+"] | "+str(pid)+" ",
        'state': "Unique software providing huge functionality and easy setup, with a fast and friendly support team.",
        'start_timestamp': time.time(),
        'large_image_key': "nl",
        'large_image_text': 'About to smash something'
    })
    for i in range(3):      
